...
public class PspActivity extends Activity {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (mRecorderView.getTimeCount() > 1)
                        ...
                        mRecorderView.stop();
                        Toast.makeText(PspActivity.this, "视频录制时间太短", Toast.LENGTH_SHORT).show();
                    }
                }
                return true;
            }
    ...
    private void finishActivity() {
        if (isFinish) {
            mRecorderView.stop();
            // 返回到播放页面
            Intent intent = new Intent(PspActivity.this, PlayVideoActivity.class);
